...
final readonly class PayslipAuditServiceService implements PayslipAuditServiceInterface
{
    ...
    public function generate(Payslip $payslip): Audit
    {
        ...
        $notFilledProperties = $this->payslipFieldsSanityService->execute($payslip);

        return new Audit($arithmeticConsistency, $socialSecurityCoherence, $heuristicIntegrity, $notFilledProperties);
    }
}
